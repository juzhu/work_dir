...
postmanLogin  = function(user_name, passwd, cb) {
  var sql = "select user_pri, uid, user_name, passwd, if(md5('" + passwd + "')=passwd, 1, 0) as loginok  from " + config.mysql_db + "." + config.post_man_tb + " where user_name='" + user_name + "';";
      ...
      client.query(sql, function(err, results) {
        if (!err) {
          ...
          if (results.length == 1 && results[0].loginok == 1) {
            cb(false, true, results[0].user_pri);
          } else {
